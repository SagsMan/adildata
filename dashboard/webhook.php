<?php
/**
 * Monnify Webhook Handler — Adildata Web Dashboard
 * URL: https://adildata.com.ng/dashboard/webhook.php
 *
 * Must return HTTP 200 or Monnify retries endlessly.
 */
...
if ($event_type === 'SUCCESSFUL_TRANSACTION' && !empty($email) && !empty($reference)) {
    ...
    // ── Insert wallet history (net credited amount, not gross) ────────────────
    ...
    mysqli_query($connect, "UPDATE payment_history_tbl SET status = 1, reason = 'monnify_success' WHERE trans_id = '$refSafe' LIMIT 1");

    @file_put_contents($log_file,
        date('Y-m-d H:i:s') . " | PROCESSED email=$email amt_paid=$amt_paid credited=$amount_to_add wallet_ok=" . ($walletOk?'Y':'N') . " history_ok=" . ($historyOk?'Y':'N') . " new_balance=$new_balance\n",
        FILE_APPEND | LOCK_EX);
    ...
    if ($walletOk && $historyOk) {
        ...
        http_response_code(500);
        echo json_encode(['status' => 'DB_WRITE_ERROR', 'wallet_ok' => $walletOk, 'history_ok' => $historyOk]);
    }

} elseif (in_array($event_type, ['FAILED_TRANSACTION', 'REVERSED_TRANSACTION', 'EXPIRED_TRANSACTION'])) {
    $refSafe = mysqli_real_escape_string($connect, $reference);
    mysqli_query($connect, "UPDATE payment_history_tbl SET status = 2, reason = '" . strtolower($event_type) . "' WHERE trans_id = '$refSafe' LIMIT 1");
