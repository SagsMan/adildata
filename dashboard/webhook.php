 ...
 * Triggered by Monnify on successful/failed payment events.
 */

// Read raw payload first (must be done before any output)
...
// Fallback to hardcoded if not in DB (same value — safety net)
if (empty($SECRET_KEY)) {
...
if ($event_type === 'SUCCESSFUL_TRANSACTION' && !empty($email) && !empty($reference)) {
    ...
    // ── Get current wallet balance for this user ──────────────────────────────
    ...
    // ── Update payment_history_tbl if record exists ───────────────────────────
    ...
    // ── Log failed/reversed transaction ───────────────────────────────────────
    $refSafe   = mysqli_real_escape_string($connect, $reference);
    $emailSafe = mysqli_real_escape_string($connect, $email);
    mysqli_query($connect,
