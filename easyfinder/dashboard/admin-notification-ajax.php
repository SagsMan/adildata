...
/* ══════════════════════════════════════════════════════════════════════════ */
/* RESEND (to failed / pending users)                                         */
/* ══════════════════════════════════════════════════════════════════════════ */
if ($action === 'resend' && $id > 0) {
    ...
    /* Get users who haven't been successfully delivered */
    ...
    if ($users_r) {
        while ($u = mysqli_fetch_assoc($users_r)) {
            ...
            $uid = intval($u['id']);
            /* Upsert: update existing failed/pending or insert new */
    ...
    mysqli_close($conn);
    $dest = $id > 0 ? "admin-notification-detail?id=$id&resent=$resent" : 'admin-notifications';
    header('Location: ' . $dest); exit;
}

/* ══════════════════════════════════════════════════════════════════════════ */
/* EXPORT CSV (single notification)                                           */
/* ══════════════════════════════════════════════════════════════════════════ */
if ($action === 'export' && $id > 0) {
    ...
    $filename = 'notification_' . $id . '_delivery_' . date('Ymd_His') . '.csv';
    ...
    fputcsv($out, ['Notification Report']);
...
/* ══════════════════════════════════════════════════════════════════════════ */
/* EXPORT ALL (all notifications summary)                                     */
/* ══════════════════════════════════════════════════════════════════════════ */
if ($action === 'export_all') {
    $filename = 'notifications_report_' . date('Ymd_His') . '.csv';
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename="' . $filename . '"');
    header('Pragma: no-cache');
    header('Expires: 0');
    ...
    $out = fopen('php://output', 'w');
    fputcsv($out, ['Notifications Report - Generated: ' . date('d M Y H:i')]);
    fputcsv($out, []);
    ...
    if ($all_r) {
        while ($row = mysqli_fetch_assoc($all_r)) {
            ...
            fputcsv($out, [
                $i++, $row['id'], $row['title'], $row['notif_type'], $row['priority'], $row['status'],
...
/* ══════════════════════════════════════════════════════════════════════════ */
/* STATS (JSON — for dashboard auto-refresh)                                  */
/* ══════════════════════════════════════════════════════════════════════════ */
if ($action === 'stats') {
    header('Content-Type: application/json');
...
/* ══════════════════════════════════════════════════════════════════════════ */
/* SEND SCHEDULED (called via cron or manual trigger)                         */
/* ══════════════════════════════════════════════════════════════════════════ */
